...
import java.io.*;
import java.net.*;
public class ConnectionSever extends JPanel implements ActionListener ,MouseListener,KeyListener, Runnable{
	...
	//解析不了网络地址异常，输出异常，中断异常
	public ConnectionSever(int count,int n)throws UnknownHostException, IOException,InterruptedException{
		setSize(600, 600);
		this.n = n;
		mapUtil = new Map(count, n);
		map = mapUtil.getMap();//获取初始时，图片种类为count,行列数为n的地图信息
        map2=new int[10][10];
        sever = new ServerSocket(5500);
        s=sever.accept();
	...
	public void drawSelectedBlock(int x, int y, Graphics g) {
		Graphics2D g2 = (Graphics2D) g;//生成Graphics对象,画笔宽度为1
		g2.setColor(Color.ORANGE);
		g.drawRect(x+1, y+1, 48, 48);
	}
	//画线，此处的x1,y1,x2,y2二维数组下标
	public void drawLink(int x1, int y1, int x2, int y2) {//用于消除方块和画线
		...
		count+=2;//消去的方块数目+2
		Connection.textField.setText(count+"");
		try {
