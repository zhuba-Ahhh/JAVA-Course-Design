...
//        borderPane.setBackground(new Background(new BackgroundFill(Color.WHITE,new CornerRadii(2),null)));
        ...
        borderPane.setBottom(getBottomPane());
        borderPane.setCenter(getCenterPane());
        borderPane.setBackground(new Background(new BackgroundFill(Color.BLACK,null,null)));//颜色的设置
    ...
    private BorderPane getCenterPane() {
        ...
        lab1.setTextFill(Color.WHITE);//文字：暗红色
        BorderStroke bs = new BorderStroke(
                Color.rgb(0,150,180),//四个边的颜色
                Color.rgb(0,150,180),
                Color.rgb(0,150,180),
                Color.rgb(0,150,180),
...
//        File lrcFile = new File(path.substring(0,path.lastIndexOf("."))+ ".lrc");
    ...
    private BorderPane getBottomPane() {
        ...
        lab3.setOnMouseClicked(e -> {
            ...
            //停止光盘的旋转
            this.timeline.stop();
            //1 : 列表循环；2. 顺序播放  3.单曲循环

            if (this.playMode==1 || this.playMode ==2){  //如果状态是 列表循环 或者 顺序播放 索引加一
                this.currentIndex+=1;
            }
            if (this.playMode == 3){ // 如果状态是 单曲循环 则索引不变
                this.currentIndex +=0;
            }
...
//        sldVolume.setPrefHeight(10);
    ...
    private BorderPane getLeftPane() {
        ...
        //3.日期：Label
        Label labDate = new Label("博学笃行，盛德日新");
